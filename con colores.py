...
class ImageDownloader:
    def __init__(self, master):
        ...
        self.label_format = ttk.Label(self.master, text="Formato de la imagen:")
        self.format_var = tk.StringVar(value="PNG")  # valor predeterminado
        self.format_menu = ttk.OptionMenu(self.master, self.format_var, "PNG", "JPEG", "GIF", "WEBP")
        ...
        self.label_format.grid(row=2, column=0, padx=5, pady=5)
        self.format_menu.grid(row=2, column=1, padx=5, pady=5)
        self.button_download.grid(row=3, column=0, padx=5, pady=5)
        self.button_exit.grid(row=3, column=1, padx=5, pady=5)
    ...
    # Obtener la URL de la imagen, el nombre de archivo y el formato
        url = self.entry_url.get()
        filename = self.entry_name.get()
        format = self.format_var.get()

        print("Formato seleccionado:", format)
    ...
    # Guardar la imagen con el formato seleccionado
        img.save(filename + '.' + self.format_var.get(), format=self.format_var.get())


    # Mostrar un mensaje de éxito
        messagebox.showinfo("Descarga exitosa", f"La imagen se ha descargado como {filename} en formato {format}")
